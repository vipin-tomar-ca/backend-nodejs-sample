...
const metrics = metrics_1.MetricsUtils.getInstance();
router.get('/health', async (req, res) => {
    try {
        const healthResult = await health_1.HealthCheckUtils.runHealthChecks();
...
router.get('/ready', async (req, res) => {
    try {
        const healthResult = await health_1.HealthCheckUtils.runHealthChecks();
        const criticalChecks = Object.values(healthResult.checks).filter(check => check.status === 'unhealthy');
...
router.get('/rate-limit/test', async (req, res) => {
    try {
        ...
        const allowed = await rateLimiter.isAllowed(identifier, 10, 60000);
        if (!allowed) {
...
router.get('/info', (req, res) => {
    try {
        const distributedConfig = config_1.config.get('distributed');
        const healthStats = health_1.HealthCheckUtils.getHealthStats();
        res.status(200).json({
            service: {
